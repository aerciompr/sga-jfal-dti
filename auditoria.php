...
// Proteção de Acesso: Apenas administradores e recepcionistas podem auditar os dados
if (!isset($_SESSION['usuario_id']) || !in_array($_SESSION['usuario_role'], ['admin', 'recepcao'])) {
...
<body class="bg-gray-950 text-gray-100 min-h-screen flex flex-col">
    ...
    <header class="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between shadow-md">
        <div class="flex items-center space-x-3">
            ...
            <span class="bg-gray-800 text-gray-400 text-xs px-2.5 py-1 rounded-full uppercase font-semibold">Auditoria Geral</span>
        </div>
        <div class="flex items-center space-x-4">
            <a href="dashboard.php" class="text-xs text-blue-400 hover:text-blue-300 border border-blue-900/60 bg-blue-950/30 px-3 py-1.5 rounded-lg transition font-semibold">Painel Central</a>
            <a href="recepcao.php" class="text-xs text-emerald-400 hover:text-emerald-300 border border-emerald-900/60 bg-emerald-950/30 px-3 py-1.5 rounded-lg transition font-semibold">Recepção</a>
            <a href="gerenciar_pauta.php" class="text-xs text-indigo-400 hover:text-indigo-300 border border-indigo-900/60 bg-indigo-950/30 px-3 py-1.5 rounded-lg transition font-semibold font-medium font-bold">Gerenciar Pautas</a>
            <span class="text-sm text-gray-300">Olá, <strong class="text-white"><?= htmlspecialchars($_SESSION['usuario_name']) ?></strong></span>
            <a href="logout.php" class="bg-red-950/40 hover:bg-red-900/40 border border-red-900/60 text-red-400 text-xs px-3 py-1.5 rounded-lg transition font-semibold">Sair</a>
        </div>
    </header>
